...
log_ch <- get_logic_errors_categorical(data = loop_3, logic_frame = logic_checks, id_col = "_uuid", index_col = "_index")
...
get_logic_applied <- function(data, logic_frame_issues, idvar){
  ...
  id_logic <- !!sym(logic_frame_issues$idvar)
  nam_ds <- setdiff(nam_ds, "_uuid")


  for(vi in nam_logic){
    id_logic_i <- !!sym(logic_frame_issues$idvar)[logic_frame_issues$variable_to_change == vi]

    for(id in id_logic_i){

    main_ds_rec[ which(!!sym(logic_frame_issues$idvar) == id), vi] <- logic_frame_issues$correct_answer[which(!!sym(logic_frame_issues$idvar) == id & logic_frame_issues$variable_to_change == vi)]
...
data2 <- get_logic_applied(main_ds, logic_issues_main, idvar="_uuid")
```
